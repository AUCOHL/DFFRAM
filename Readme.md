...
<em> Note: The project will consider the creation of a custom router if automatic routing using open-source global and detailed routers do not give good results, though, they are, so far! </em>

A beta version of the compiler is under the [`Compiler/`](./Compiler) directory. Check [its Readme](./Compiler/Readme.md) for more info. The Compiler relies on basic building blocks to construct the layout of different RAM/RF/Cache configurations. The following shows how a 32x32 memory (DFF based) is constructed.
![](./docs/img/ram_ex.png)
...
The layout of 2048x32 (8 Kbytes) RAM as generated by teh compiler is shown below:
![8kbytes](./Compiler/docs/img/8kb_layout.png).
