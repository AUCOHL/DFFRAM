DFFRF_instantiation_2R1W_legacy = """
/*
...
`define     RWIDTH          {word_size}
`define     RCOUNT          {word_num}
...
module tb_DFFRF{word_num}x{word_size}_2R1W;

    localparam RWIDTH = `RWIDTH
    localparam M_SZ = {word_num};
    ...
    DFFRF_2R1W #(.RWIDTH(RWIDTH), .RCOUNT(RCOUNT), .R0_ZERO(1) )
    (
	.RA(RA),
        .RB(RB),
        .RW(RW),
    ...
    initial begin
        $dumpfile("tb_DFFRF{word_num}x{word_size}_2R1W.vcd");
        $dumpvars(0, tb_DFFRF{word_num}x{word_size}_2R1W);
        @(done) $finish;
    end

     /* Memory golden Model */
    reg [(RWIDTH-1):0]      DFFRF[(M_SZ)-1 : 0];
    reg [(RWIDTH-1):0]      DFFRF_DATA_WRITE;
    reg [(RWIDTH-1):0]      DFFRF_DATA_READA;
    reg [(RWIDTH-1):0]      DFFRF_DATA_READB;

    always @(posedge CLK) begin
        DFFRF_DATA_READA <= DFFRF[RA];
        DFFRF_DATA_READA <= DFFRF[RB];
        if(WE) DFFRF[RW] <= DFFRF_DATA_WRITE[RW];
    end
"""
